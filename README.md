...
| Flag | Descricao |
| --- | --- |
| `--companies` | nomes separados por virgula (match exato na base; sem match, a empresa e ignorada com aviso) |
| `--output` | JSON de saida (o CSV e derivado, `<nome>.csv`) |
...
Saida: imprime `Found N jobs` + lista (titulo, empresa, local, URL) e grava
`jobs.json` / `jobs.csv`.
...
- `internship` e preenchido pelo adapter via heuristica (`filters.py`, termos
  EN/PT/DE: intern, trainee, student, working student, Werkstudent, Praktikum, iXp...).
...
├── filters.py      # heuristica is_internship
└── cli.py          # entry point `internship-finder`
