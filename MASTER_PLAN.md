...
- `main` = `f5da0d3` (18/09 — PRs #66/#67/#68: fechamento final documental +
  ...
  **#70 = Fase 2 — Telegram Daily Digest**; **#71 = Fase 3 — interface do
  ...
  "por que este score" — ver Log 18/09): **CI verde — 25/25 scripts,
  1.204 checks, 0 falhas**; ranking público em
  https://rios-vini.github.io/internship-finder/ (branch `gh-pages`, só
  `index.html`, gerada por `scripts/publish_pages.py`); mensagem diária do
  Telegram agora é o **digest do ranking** (perfil/critérios ativos, novas
  vagas no Top 30, Top 5, mudanças e link — `scripts/ranking_digest.py`,
  cron com `--always-notify`); cron diário `0 9 * * *`
  UTC (= 06:00 America/Sao_Paulo, UTC-3 fixo — Vixie não suporta `CRON_TZ`;
  ver Log 18/09). Histórico do dia: `ea59f07` (PR #65 = última onda de
