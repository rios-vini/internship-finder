...
| 5 | **Persistência SQLite** + `first_seen`/`last_seen`/`active`/`archived` | ✅ **implementado 01/09** (PR #10, main via commit `e683cd8`) — desbloqueado 31/08 (decisão do dono; era ON HOLD desde 13/08) | `sqlite3` stdlib, sem Postgres/Redis/ORM (decisão do dono 31/08 preservada; parecer A autorizava; pré-requisito do histórico). Verificado (06/09): `src/internship_finder/storage/sqlite_store.py` (`SqliteStore`, DDL com `application_deadline` + `LIFECYCLE_COLUMNS` first/last_seen/active/archived); flag `--sqlite PATH` no `cli.py` (grava o run; falha nunca derruba a coleta — except noqa BLE001); `scripts/test_sqlite.py` no array do CI. |
...
| 30 | Reavaliar range `>=0.3.0` do ats-scrapers quando o upstream evoluir (maturidade da release) | ⏳ **checado em 2026-09-06** | P3 #19 eliminou a ref de PR `ae0ad53`; restou o range aberto no pyproject. Na próxima release do ats-scrapers (≥0.4.0): revalidar expose de `application_deadline` + suíte e decidir cap (`==0.3.*`) se a API mudar. Gate: nova release observada no PyPI ou falha de install/import. **Checagem 06/09 (15:01 UTC): PyPI última release 0.3.0 (releases 0.1.0/0.2.0/0.3.0), expose presente na instalação, import OK, range `>=0.3.0` mantido, gate não disparado — zero mudança de código (ver Log de mudanças).** |
