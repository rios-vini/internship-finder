...
## 1. Estado verificado em 2026-09-20

### Main público (GitHub)
- `main` = `1367d3c4` (20/09 — **Fase 6 — Candidate Fit + Application
  Intelligence + Mobile UX**, PR #75 squash; **CI verde — 28/28 scripts, 0
  falhas**): ranking público em
  https://rios-vini.github.io/internship-finder/ agora com **chips de
  candidatura por vaga** (deadline do empregador/idioma/visto/readiness),
  bloco **"sinais de candidatura e possíveis problemas"** (fit, work
  authorization, deadline primeira classe — valor SuccessFactors = validade
  de feed da fonte, nunca prazo; quality flags; freshness), seção **"Como
  este ranking funciona"** com os pesos REAIS das constantes (nada
  duplicado), **filtros novos** (vagas expirando ≤2/≤7/≤14 — só deadline
  confiável —, work auth, idioma, Top 30, quality flags) com contador, e
  **mobile cards** (<760px, sem scroll horizontal); **alemão classificado
  por exigência no TEXTO** (exigido −2,0 / preferido −0,5 / neutro; EN +1,5
  inalterado); `app_intel.py` novo; status pessoal = evolução futura; Fase 7
  (Company & Location Intelligence) NÃO implementada. Ver Log 20/09 +
  `docs/relatorio_fase6_candidate_fit.md`. Antecedentes: `5e1cc95` (Fase 5
  #74), `9ad6bf8` (Fase 4 #73), `f5da0d3` (Fase 3 hotfix #72),
  `0939f05` (Fase 3 #71).
